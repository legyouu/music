...
def song(_, message):
    ...
    m = message.reply("🔎 **يتم البحث انتظر**...")
    ...
    try:
        ...
        rep = f"**🎧 الرفع بواسطه: [Java](https://t.me/JAI6H)**"
...
async def vsong(client, message):
    ...
    try:
        ...
        print(e)
    try:
        msg = await message.reply("📥 **يتم تحميل الفديو..**")
...
async def lyrics(_, message):
    try:
        if len(message.command) < 2:
            await message.reply_text("» **اكتب الاسم الصحيح للاغنيه او الفيد.**")
            return
        query = message.text.split(None, 1)[1]
        rep = await message.reply_text("🔎 **يتم البحث انتظر...**")
        resp = requests.get(
