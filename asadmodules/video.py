...
async def vplay(c: Client, m: Message):
    ...
    if a.status != "administrator":
        await m.reply_text(
            f"📽️ To Use me, I Need to be an **Administrator** With the following **Permission**:\n\n» ❌ __Delete Messages__\n» ❌ __Restrict User__\n» ❌ __Add Users__\n» ❌ __Manage Video chat__\n\nData Is**ᴜᴘᴅᴀᴛᴇᴅ** Automatically After You **Promote me**"
        )
        return
    ...
    if replied:
        if replied.video or replied.document:
            ...
            if chat_id in QUEUE:
                ...
                await m.reply_photo(
                    photo=f"{IMG_1}",
                    caption=f"📽️ **Track added to queue »** `{pos}`\n📮 **Name:** [{songname[:10]}]({link})\n💭 **Chat:** `{chat_id}`\n🔋 **Request By:** {requester}",
            ...
            if len(m.command) < 2:
                await m.reply(
                    "**» Reply to an give something to search.**"
                )
            else:
                ...
                amaze = HighQualityVideo()
                if search == 0:
                    await loser.edit("❌ **No Result Found.**")
                else:
                    songname = search[0]
                    url = search[1]
                    ...
                    if veez == 0:
                        ...
                        if chat_id in QUEUE:
                            ...
                            await m.reply_photo(
                                photo=f"{IMG_1}",
                                caption=f"📽️ **Song Added to Queue »** `{pos}`\n📮 **Name:** [{songname[:10]}]({url})\n🔋 **Required By:** {requester}",
                                reply_markup=keyboard,
                            )
                        else:
                            try:
        ...
        if asad == 0:
            ...
            if chat_id in QUEUE:
                ...
                try:
                    ...
                    await m.reply_photo(
                        photo=f"{IMG_2}",
                        caption=f"📽️ **[Video Live]({link}) Stream Startes.**\n\n💭 **Chat:** `{chat_id}`\n\n🚨 **Status:** `Playing`\n🔋 **Request By:** {requester}",
                        reply_markup=keyboard,
                    )
                except Exception as ep:
                    await loser.delete()
